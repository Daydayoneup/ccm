...
pub fn list_project_resources(
    ...
    // Get project-scope resources whose source_path is under this project.
    // All installs (FileBased + ConfigBased) create a project-scope resource
    // record, so we only need to query the project scope.
    let results = match resource_type {
        Some(rt) => {
            let rtype = ResourceType::from_str(&rt)
                .ok_or_else(|| format!("Invalid resource type: {}", rt))?;
            db.list_resources_by_scope_and_type(&ResourceScope::Project, &rtype)
                .map_err(|e| e.to_string())?
        }
        None => {
            db.list_resources_by_scope(&ResourceScope::Project)
                .map_err(|e| e.to_string())?
        }
    }.into_iter()
    .filter(|r| r.source_path.starts_with(&project.path))
